...
from reinforcement_learning.marl.constants import REWARDS, AGENT_ACTIONS, AgentActions
...
class HostAgentEnv(gym.Env):
    ...
    def execute_action(self,action, show_action=False, name = "", reward = 0):
        ...
        if action == AGENT_ACTIONS.NORMAL_TRAFFIC:
            ...
            if not self.is_link_on and unblock_flow_delete(self.net, self.host_name): #attach_link(self.net, self.host_name):
                time.sleep(0.2)
                self.set_link_status(True)  
        ...
        if action == AGENT_ACTIONS.OUTGOING_ATTACK:
            ...
            #stop link
            if self.is_link_on and block_flow_drop(self.net, self.host_name): #detach_link(self.net, self.host_name):
                time.sleep(0.2)
