...
from reinforcement_learning.attack_detect.constants import AGENT_STATUS_ID_MAPPING, HOST_STATUS_ID_MAPPING
...
class InstantState(BaseInstantState):
    ...
    def update_statuses(self, status: str, host_statuses: dict):
        ...
        """
        status_id = AGENT_STATUS_ID_MAPPING.get(status, -1)
        self.status = {
            "id" : status_id,
            "status" : status,
