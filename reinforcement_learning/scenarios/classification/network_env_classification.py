...
class NetworkEnvClassification(NetworkEnv):     
    ...
    def update_state(self, episode = None):
        ...
        if self.gym_type == GYM_TYPE[CLASSIFICATION] and self.state is not None:    #classification
            ...
            # Clean up residual traffic from previous step to ensure only one traffic at a time
            self.clean_network_state()
    ...
    def get_discretized_state(self, state):
        # Return zeroed discrete state if nothing provided
        if state is None:
            return tuple([0] * len(self.low))

        # Unwrap common wrappers
        if isinstance(state, tuple) and len(state) == 1:
            state = state[0]

        # Coerce to numpy array for consistent handling
        try:
            arr = np.array(state, dtype=np.float32)
        except Exception:
            # fallback to global_state if available
            try:
                arr = np.array(self.global_state.state, dtype=np.float32)
            except Exception:
                arr = np.zeros(len(self.low), dtype=np.float32)

        # If scalar (0-dim) or not the expected size, try to recover a 1-D array
        if arr.ndim == 0:
            try:
                arr = np.array(self.global_state.state, dtype=np.float32)
            except Exception:
        ...
        if arr.size != len(self.low):
            flat = arr.flatten()
            if flat.size < len(self.low):
                padded = np.zeros(len(self.low), dtype=np.float32)
                padded[:flat.size] = flat
                arr = padded
            else:
                arr = flat[: len(self.low)]

        # Digitize per-dimension into discrete bins
        discrete_state = []
        for i, val in enumerate(arr):
            bin_index = int(np.digitize(float(val) + 1.0, self.bins[i]) - 1)
